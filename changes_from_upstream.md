...
One further consequence: Coolify only auto-discovers a variable when it is the *entire* value of a compose key, so none of the `API_SUMMARY_*` variables appear in the Environment Variables UI on their own — they are embedded inside the larger `API_SUMMARY` string. They work normally once added by hand, and `configuration_options.md` tells operators to do so.
