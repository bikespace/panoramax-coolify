...
`API_SUMMARY` is the one whose value is not a simple scalar, and it is parameterised field by field — `API_SUMMARY_NAME`, `API_SUMMARY_DESCRIPTION`, `API_SUMMARY_GEO_COVERAGE`, `API_SUMMARY_COLOR`, `API_SUMMARY_LOGO`, `API_SUMMARY_EMAIL` — rather than as one variable holding the whole blob. The JSON structure stays literal in the `>-` folded block in `docker-compose.yml` and each variable fills in a piece of it. The three translated fields take the *contents* of their translation object (`"en": "…", "fr": "…"`) with the enclosing braces supplied by the compose file, so an operator can publish whatever set of languages they want without editing the compose file.

> **Never put `{` or `}` inside a `${VAR:-default}` in `docker-compose.yml`.** This is a hard constraint on this repo, and the reason `API_SUMMARY` is split up rather than exposed as a single variable. Docker Compose itself handles brace-containing defaults correctly, but Coolify parses the compose file with its own parser, which does not: given `${API_SUMMARY:-{ "color": … }}` it reads part of the default as a *second* variable name and emits it as a build argument, producing `--build-arg "color": "#abcdef", …`. The unbalanced quotes then kill the build command with `bash: -c: line 1: unexpected EOF while looking for matching '"'`. Colons and dashes in defaults are fine (several URLs and `CC-BY-SA-4.0` rely on that); braces are not. Related upstream bugs: [coollabsio/coolify#8851](https://github.com/coollabsio/coolify/issues/8851), [#3674](https://github.com/coollabsio/coolify/issues/3674), [#7542](https://github.com/coollabsio/coolify/issues/7542).

**`INSTANCE_NAME` sets the website only.** Upstream had the website fall back to `A geovisio instance` while the API's `API_SUMMARY` fell back to `A Panoramax instance`, so an operator who left it unset got different names in the site header and the federation metadata. Both defaults now read `A Panoramax instance`, but they are two independent variables — `INSTANCE_NAME` for the website, `API_SUMMARY_NAME` for the API — because a default that wired one into the other would have to contain `${INSTANCE_NAME:-…}`, and that means braces inside a default. A fresh deploy therefore looks consistent out of the box, but an operator who renames their instance must set both.
