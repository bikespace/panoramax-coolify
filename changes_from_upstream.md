...
`API_SUMMARY` is the one whose value is not a simple scalar, and it is parameterised field by field and language by language — `API_SUMMARY_NAME_EN`, `API_SUMMARY_NAME_FR`, `API_SUMMARY_DESCRIPTION_EN`, and so on, plus `API_SUMMARY_COLOR`, `API_SUMMARY_LOGO` and `API_SUMMARY_EMAIL` — rather than as one variable holding the whole blob. All of the JSON structure stays literal in the `>-` folded block in `docker-compose.yml`, and each variable supplies only the plain text between a pair of quotes. Adding or removing a language is therefore a compose edit rather than a variable, which is the deliberate trade for the two constraints below.

> **In `docker-compose.yml`, a `${VAR:-default}` must contain no `{`, `}` or `"`, and no variable may carry JSON punctuation in its value.** Both halves are Coolify's limitations, not Docker Compose's — Compose handles all of it correctly — and both were hit in production on this branch:
>
> - **Braces in a default break the build.** Given `${API_SUMMARY:-{ "color": … }}`, Coolify's compose parser reads part of the default as a *second* variable name and emits it as a build argument: `--build-arg "color": "#abcdef", …`. The unbalanced quotes kill the build command with `bash: -c: line 1: unexpected EOF while looking for matching '"'`. Colons and dashes are fine — several URL defaults and `CC-BY-SA-4.0` rely on that — braces are not.
> - **Quotes in an operator-set value break the API.** A value containing `"` set through the Coolify UI does not reach the container intact, so the assembled string is no longer valid JSON and `api` crash-loops on startup with `Parameter API_SUMMARY is not recognized`. Quotes in the compose file's *defaults* are unaffected; it is only values coming from the UI that are mangled.
>
> Related upstream bugs: [coollabsio/coolify#8851](https://github.com/coollabsio/coolify/issues/8851), [#3674](https://github.com/coollabsio/coolify/issues/3674), [#7542](https://github.com/coollabsio/coolify/issues/7542).

One further consequence: Coolify only auto-discovers a variable that is the *entire* value of a compose key, so nothing embedded inside the `API_SUMMARY` string is listed in its Environment Variables UI. Each `API_SUMMARY_*` variable is therefore also declared on its own line in the `api` service, purely to be discovered. Those declarations carry no default, so an unset variable simply shows blank in the UI. They cannot carry the real defaults: Compose resolves `${VAR}` against the process environment and the `--env-file`, never against a sibling key in the same `environment:` block, so a default written there would still leave `""` inside `API_SUMMARY`. It would appear to work under Coolify, which persists discovered defaults into the env file it passes to Compose, while silently breaking a plain `docker compose up`.

**`INSTANCE_NAME` sets the website only.** Upstream had the website fall back to `A geovisio instance` while the API's `API_SUMMARY` fell back to `A Panoramax instance`, so an operator who left it unset got different names in the site header and the federation metadata. Both defaults now read `A Panoramax instance`, but they are two independent variables — `INSTANCE_NAME` for the website, `API_SUMMARY_NAME_EN` for the API — because a default that wired one into the other would have to contain `${INSTANCE_NAME:-…}`, and that means braces inside a default. A fresh deploy therefore looks consistent out of the box, but an operator who renames their instance must set both.
