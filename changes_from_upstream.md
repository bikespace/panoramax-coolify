...
`API_SUMMARY` is the one whose value is not a simple scalar, and it is parameterised field by field and language by language — `API_SUMMARY_NAME_EN`, `API_SUMMARY_NAME_FR`, `API_SUMMARY_DESCRIPTION_EN`, and so on, plus `API_SUMMARY_COLOR`, `API_SUMMARY_LOGO` and `API_SUMMARY_EMAIL` — rather than as one variable holding the whole blob. All of the JSON structure stays literal in the `>-` folded block in `docker-compose.yml`, and each variable supplies only the plain text between a pair of quotes. Adding or removing a language is therefore a compose edit rather than a variable. That shape is not a preference: Coolify mangles a `{`, `}` or `"` in a variable's default or in an operator-set value, and it lists a variable in its UI only when the variable is the whole value of a compose key — so each `API_SUMMARY_*` is additionally declared on its own line in the `api` service, without a default, purely to be discovered. See [`CONTRIBUTING.md` → Coolify behaviours to know](./CONTRIBUTING.md#coolify-behaviours-to-know) for what each of those behaviours does when you hit it.
